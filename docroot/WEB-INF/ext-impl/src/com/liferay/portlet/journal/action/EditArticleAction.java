...
public class EditArticleAction extends PortletAction {
	...
	protected Object[] updateArticle(ActionRequest actionRequest)
		...
		if (cmd.equals(Constants.ADD)) {
			...
			//avoid static contents
			if(articleContent != null && !articleContent.contains("<static-content")){
				
				ThemeDisplay themeDisplay = (ThemeDisplay)actionRequest.getAttribute(WebKeys.THEME_DISPLAY);
				String url = PortalUtil.getLayoutURL(themeDisplay.getLayout(),themeDisplay);
				
				String additionalContent = "\t<dynamic-element name=\"URL\" type=\"text\" index-type=\"keyword\" index=\"1\">\n" +
		                "\t\t<dynamic-content language-id=\"en_US\"><![CDATA["+url+"]]></dynamic-content>\n" +
		                "\t</dynamic-element>\n";
				
				content = articleContent.substring(0,articleContent.indexOf("</root>")).concat(additionalContent).concat("</root>");
				
				System.out.println("content add : " + content);
			...
			if(articleContent != null && !articleContent.contains("<static-content")){
				...
				System.out.println("content update : " + content);
